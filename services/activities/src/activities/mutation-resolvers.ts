...
export const activityCreate: MutationResolvers["activityCreate"] = async (
  ...
  try {
    ...
    throw new Error(`Unable to save activity: ${error}`);
  }
};

export const activityUpdate: MutationResolvers['activityUpdate'] = async (_, args: MutationActivityUpdateArgs) => {
  ...
  try {
    ...
    throw new Error(`Unable to update activity: ${error}`);
  }
};
